...
addBackAddress (Ptr<Node> pgw, Ptr<NetDevice> ueLteNetDev, Ipv4Address addr)
{
  ...
  Ptr<NrUeNetDevice> uedev = ueLteNetDev->GetObject<NrUeNetDevice> ();
...
//Install and get the pointers to the NetDevices
  NetDeviceContainer enbLteDevs = nrHelper->InstallGnbDevice (enbNode, allBwps);
  NetDeviceContainer ueLteDevs = nrHelper->InstallUeDevice (NodeContainer (nodes.Get(0)), allBwps);

  randomStream += nrHelper->AssignStreams (enbLteDevs, randomStream);
  randomStream += nrHelper->AssignStreams (ueLteDevs, randomStream);

  // Set the attribute of the netdevice (enbLteDevs.Get (0)) and bandwidth part (0)
  nrHelper->GetGnbPhy (enbLteDevs.Get (0), 0)->SetAttribute ("Numerology", UintegerValue (numerologyBwp1));

  for (auto it = enbLteDevs.Begin (); it != enbLteDevs.End (); ++it)
    {
      DynamicCast<NrGnbNetDevice> (*it)->UpdateConfig ();
    }

  for (auto it = ueLteDevs.Begin (); it != ueLteDevs.End (); ++it)
    {
  ...
  nrHelper->AttachToClosestEnb (ueLteDevs, enbLteDevs);
  //nrHelper->Attach (ueLteDevs.Get(0), enbLteDevs.Get (0));
